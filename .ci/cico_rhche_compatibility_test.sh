...
if [[ $PR_EXISTS -eq 1 ]]; then
	...
	echo "Pull request for tracking changes of version $CHE_VERSION was not found."
fi
...
if [ $RETURN_CODE != 0 ]; then
	url=$(curl -s https://api.github.com/repos/redhat-developer/rh-che/pulls?state=open | jq '.[] | select(.title == "$RELATED_PR_TITLE") | .url')
	...
	curl -X POST -s -L -H "Authorization: token $(echo ${FABRIC8_HUB_TOKEN}|base64 --decode)" https://api.github.com/repos/redhat-developers/rh-che/issues/2/comments -d "{\"body\": \"$message\"}"
fi
set -e
